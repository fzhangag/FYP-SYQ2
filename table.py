...
table.config['SQLALCHEMY_TRACK_MODIFICATIONS'] = True
table.config['SQLALCHEMY_DATABASE_URI'] = 'sqlite:///db/table.db'
db = SQLAlchemy(table)
3
...
def random_integer():
    ...
    run = database.exist_userid(rand)
    while run:
        rand = randint(min_, max_)
        run = database.exist_userid(rand)
...
class User(db.Model):
    ...
    def __init__(self, username, name, password, gender, birth, mobile, address, state, goal, modify, second_sentiment,
                 first_sentiment, guest, last_modify):
...
class employment(db.Model):
    ...
    def __init__(self, userid, employee_name, employer_name, company_name, company_address, employee_address, job, duty,
                 full, work_hour, salary, annual_leave, commence, ddl, remote, work_place, probationary, duration,
                 advance, insurance, commission, num_temp):
        self.userid = userid
...
class divorce(db.Model):
    ...
    def __init__(self, userid, mother_name, father_name, mother_hkid, mother_address, father_hkid, father_address,
                 mother_age, father_age, marriage_date, num_child, child_name, born_date, university,
                 children_maintence, child_main_who, child_main_amount, child_main_modify, custody, major_charge,
                 leave_hk, where_live, when_live, child_access, spousal_maintance, spousal_who, spousal_type,
                 spousal_much, mother_asset, father_asset, num_temp):
        self.userid = userid
