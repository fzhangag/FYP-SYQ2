function getPanelContent(type){
    ...
    if(type=='calendar'){
        ...
        str+='<form class="layui-form layui-form-child_info" action=""  style="margin:5% 10% 10%;">\n' +
            ...
            '        <h4>Full Name :</h4>\n' +
            '        <div class="layui-input-inline">\n' +
            '          <input type="text" id="child_name" name="child_name" autocomplete="off" class="layui-input">\n' +
            '        </div>\n' +
            '    </div>\n' +
            '    <div class="layui-form-item">\n'+
            '        <h4>Date of Birth :</h4>\n' +
            '        <div class="layui-input-inline">\n' +
            '          <input type="text" id="test2" name="test2"  placeholder="Please choose a date" autocomplete="off" class="layui-input">\n' +
            '        </div>\n' +
            '    </div>\n' +
        ...
        str+='<form class="layui-form layui-form—spousal" action=""  style="margin:5% 10% 10%;">\n' +
            '    <div class="layui-form-item">\n' +
            '        <div class="layui-form-item">\n' +
            ...
            '             <div class="layui-input-inline" >\n' +
            '                 <input type="number" name="hide_spousal_amount" id="spousal_amount" autocomplete="off" class="layui-input">\n' +
            '             </div>\n' +
            '        </div>\n' +
            '     </div>\n' +
            '</form>';
        ...
        str+='<form class="layui-form layui-form-probationary" action=""  style="margin:5% 10% 10%;">\n' +
            '    <div class="layui-form-item">\n' +
            '        <div class="layui-form-item">\n' +
        ...
        str+='<form class="layui-form layui-form-address" action=""  style="margin:5% 10% 10%;">\n' +
            ...
            '          <h4> Address:</h4><br>\n' +
            '          <div class="layui-input-inline" >\n' +
            '             <input type="text" name="address_input_field" id="pac-input" required  lay-verify="required" placeholder="Enter your location" autocomplete="off" class="layui-input" style="width:500px">\n' +
            '          <div id="map"></div>\n' +
            '          </div>\n' +
            '    </div>\n' +
            '</form>';
...
$().ready(
    ...
    //-----------------------------------Start of layui.use---------------------------------------------
        ...
        function displayPanel(str,btnName,response,type){
          layer.open({
           ...
           yes:function(index,layero){
                ...
                var close=check(type);
                for(var i=2; i < arr.length; i++) {
                    if(arr[i]['value']==''){
                        if((arr[i]['name'].indexOf('hide')>-1&&show==true)||(arr[i]['name'].indexOf('hide')==-1))
                            close=false;
                    }else{
                        msg+=arr[i]['value']+';';
                    }
                ...
                var append_0={'child_info':[2,';0'],'child_further_info':[3,';0;0'],'child_main_info':[2,';0;0;0'],'spousal':[1,';0;0;0']};
                if(type in append_0){
                    if(msg.split(';').length==append_0[type][0]){
                        msg+=append_0[type][1];
                    }
                }
                if(close==true){
                    ...
                    $(".layui-form-"+type)[0].reset();
                    form.render();
                    return false;
              },success: function(index,layero){
                    $(".layui-layer-title").css({ height: "75px", "text-overflow": "initial","white-space":"normal","padding":"0 20px 0 20px", "line-height":"25px"});
        ...
        $("#tt").click(function(e){
              var btnName_1=['Submit','Reset'];
              var btnName_2=['Yes','No'];
              var btnName_3=['Update','Submit Again'];
              var btnName_4=['Father','Mother'];
              var str=getPanelContent('job');
              var dic={'0':"Man hi",'1':'Director-heyheyh','2':'Director heyheyh','4':'Director heyheyh'};
              displayPanel(create_job_title_panel(dic),btnName_1,'Hi there, nice to meet you! you!!','job');
        ...
        $("#re-open").click(function(e){
            ...
            if(widget!=''){
                        ...
                        var str=getPanelContent(widget);

                        checkDuplicateLayer('layui-form');

                        if(widget=='calendar'||widget=='child_further_info'){
                            displayPanel(str,btnName_1,response,widget);
                            initCanlendar(calendarTag[widget]);
                        }else if(widget=='remote_specific'||widget=='address'){
                            displayPanel(str,btnName_1,response,widget);
                            initMap();
                        }else if(widget=='child_info'){
                            displayPanel(str,btnName_1,response,widget);
                            initCanlendar('test2');
                        }else if(widget=='name_update'||widget=='couple'||widget=='yes_or_no'){
                            displayPanel_simple_type(str,btn_for_simple_panel[widget],response,widget);
                        }else if(widget=='job'){
                            displayPanel(create_job_title_panel(job_title),btnName_1,response);
                        }else{
                            displayPanel(str,btnName_1,response,widget);
                        }
                        form.render();
...
function check(type){
    ...
    var checkList={'full_part':['job_type'],
                   'remote_specific':['remote_type'],
                   'hide_child_info':['hide_university_y_n'],
                   'child_further_info':['child_Custody','child_Charge','leave_hk'],
                   'child_main_info':['maintenance_y_n','modify_main_y_n'],
                   'hide_child_main_info':['hide_who_main'],
                   'spousal':['spousal_y_n'],
                   'hide_spousal':['hide_spousal_who','hide_spousal_type'],
                   'probationary':['prob_type']
    };
    if(type=='child_info'&&show==true){
        return checkRadio(checkList['hide_'+type]);
    }
    if(checkRadio(checkList[type])==false){
        return false;
    }
    if(type=='child_main_info'||type=='spousal'){
        if(show==true){
            return checkRadio(checkList['hide_'+type]);
        }
    }
...
function checkRadio(arr){
    for(var i in arr){
        if($('input[name='+arr[i]+']:checked').val()==null)
        {
            return false;
        }
    }
                ...
                if(jsondata["status"]=="success"){
                    ...
                    if(jsondata['type']!=''){
                        ...
                        if(jsondata['type']=='calendar'||jsondata['type']=='child_further_info'){
                            displayPanel(str,btnName_1,response,jsondata['type']);
                            ...
                            displayPanel(create_job_title_panel(dic),btnName_1,response,jsondata['type']);
                        }else{
                            displayPanel(str,btnName_1,response,jsondata['type']);
                        }
                        form.render();
...
function create_job_title_panel(dic){
    ...
    str='<form class="layui-form layui-form-job" action=""  style="margin:5% 10% 10%;">\n' +
            '    <div class="layui-form-item">\n' +
            '        <div class="layui-form-item">\n' +
            '          <div class="layui-input-inline">\n' +temp+
            '          </div>\n' +
